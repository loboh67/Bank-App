...
struct DescriptionTransactionsView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(displayTitle ?? descriptionQuery)
    ...
    private var transactionsContent: some View {
        if let errorMessage {
            ...
            Section("Transactions") {
                Text("No transactions for this description")
                    .foregroundColor(.secondary)
            }
        } else {
    ...
    private func loadData(forceReload: Bool = false) async {
        ...
        errorMessage = nil
        do {
            async let txs = APIClient.shared.fetchDescriptionTransactions(query: descriptionQuery)
            async let totalValue = APIClient.shared.fetchDescriptionTotal(query: descriptionQuery)
...
#Preview {
    NavigationStack {
        DescriptionTransactionsView(
            descriptionQuery: "coffee shop",
            displayTitle: "Coffee shop",
            mockTransactions: Transaction.previewList,
            mockTotal: CategoryTotal(amount: -87.30)
        )
    }
}
