...
struct MerchantTransactionsView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(merchantName ?? "Merchant")
    ...
    private func loadData(forceReload: Bool = false) async {
        ...
        errorMessage = nil
        do {
            async let txs = APIClient.shared.fetchMerchantTransactions(merchantId: merchantId)
            async let totalValue = APIClient.shared.fetchMerchantTotal(merchantId: merchantId)
...
#Preview {
    NavigationStack {
        MerchantTransactionsView(
            ...
            mockTotal: CategoryTotal(amount: -120.75)
        )
    }
}
